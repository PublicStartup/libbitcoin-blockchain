...
build_all()
{
rsync -av . ~/Documents/backup/libbitnew/libbitcoin-blockchain
if [ ! -d "$BUILD_DIR/libbitcoin-blockchain" ]; then
    ...
    build_from_github PublicStartup libbitcoin master $PARALLEL ${BITCOIN_OPTIONS[@]} "$@"
    build_from_github PublicStartup libbitcoin-consensus master $PARALLEL ${BITCOIN_CONSENSUS_OPTIONS[@]} "$@"
    build_from_github PublicStartup libbitcoin-database master $PARALLEL ${BITCOIN_DATABASE_OPTIONS[@]} "$@"
    build_from_travis PublicStartup libbitcoin-blockchain master $PARALLEL ${BITCOIN_BLOCKCHAIN_OPTIONS[@]} "$@"
else
    build_from_local "local re-build" $PARALLEL ${SECP256K1_OPTIONS[@]} "$@"
fi
}
